...
addon.svnrev = {}
addon.svnrev["SavedInstances.lua"] = tonumber(("$Revision: 156 $"):match("%d+"))
...
local GetSavedInstanceInfo, GetNumSavedInstances, GetSavedInstanceInfo, GetLFGDungeonNumEncounters, GetLFGDungeonEncounterInfo, GetNumRandomDungeons, GetLFGRandomDungeonInfo, GetLFGDungeonInfo, LFGGetDungeonInfoByID, GetLFGDungeonRewards, GetTime, UnitIsUnit, GetInstanceInfo, GetLFGMode, IsInInstance, SecondsToTime, GetQuestResetTime, GetGameTime, GetCurrencyListInfo, GetNumRaidMembers, GetNumPartyMembers =
	GetSavedInstanceInfo, GetNumSavedInstances, GetSavedInstanceInfo, GetLFGDungeonNumEncounters,
	GetLFGDungeonEncounterInfo, GetNumRandomDungeons, GetLFGRandomDungeonInfo, GetLFGDungeonInfo, LFGGetDungeonInfoByID,
	GetLFGDungeonRewards, GetTime, UnitIsUnit, GetInstanceInfo, GetLFGMode, IsInInstance, SecondsToTime,
	GetQuestResetTime, GetGameTime, GetCurrencyListInfo, GetNumRaidMembers, GetNumPartyMembers
...
vars.defaultDB = {
	...
	Tooltip = {
		...
		Currency102 = false,
		Currency221 = false,
		Currency301 = true, -- triumph
		Currency341 = true, -- frost
...
local function ColorCodeOpen(color)
	return format("|c%02x%02x%02x%02x", math.floor(color[4] * 255), math.floor(color[1] * 255),
...
function addon:FindInstance(name, raid)
	...
	for i = 1, GetNumSavedInstances() do
		...
		local lname, lid, _, _, _, _, _, _, _, _ = GetSavedInstanceInfo(i)
	...
	if not instance then
		...
		if not addon.warned[name] then
			...
			for i = 1, GetNumSavedInstances() do
				...
				local tlname, tlid, _, _, _, _, _, _, _, _ = GetSavedInstanceInfo(i)
				if tlname == name then lid = tlid end
			end
			print("SavedInstances: ERROR: Refresh() failed to find instance: " ..
			name .. " : " .. GetLocale() .. " : " .. (lid or "x"))
...
function addon:UpdateInstanceData()
	...
	debug("UpdateInstanceData(): completed " .. count .. " updates in " ..
	string.format("%.6f", starttime / 1000.0) .. " sec.")
...
function GetAverageItemLevel()
	ItemCount = 0; TotalILevel = 0; AVGILevel = 0;
	for i = 1, 18 do
		if not (i == 4) then
			if (GetInventoryItemLink("player", i)) then
				_, _, _, ItemLevel = GetItemInfo(GetInventoryItemLink("player", i))
				ItemCount          = ItemCount + 1; TotalILevel = TotalILevel + ItemLevel;
			end
		end
	end
	AVGILevel = TotalILevel / ItemCount;
	return AVGILevel
end
...
function addon:UpdateToonData()
	...
	for _, idx in pairs(currency) do
		...
		--_, ci.amount, _, _, _, _ = GetCurrencyInfo(idx)
			if not addon.logout then
				local _, amount, _, _, _, _ = GetCurrencyInfo(idx)
				if amount then
		...
		--   if idx == 341 then -- VP x 100, CP x 1
...
function GetCurrencyInfo(idx)
	for i = 1, GetCurrencyListSize() do
		local name, isHeader, isExpanded, isUnused, isWatched, count, extraCurrencyType, icon, itemID =
		GetCurrencyListInfo(i)
		if idx == 301 and name == "Emblem of Frost" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 341 and name == "Emblem of Triumph" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 392 and name == "Honor Points" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 390 and name == "Conquest Points" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 101 and name == "Emblem of Heroism" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 102 and name == "Emblem of Valor" then
			return name, count, icon, nil, nil, nil, nil, nil
		elseif idx == 221 and name == "Emblem of Conquest" then
			return name, count, icon, nil, nil, nil, nil, nil
		end
	end
	--local name, currentAmount, texture, earnedThisWeek, weeklyMax, totalMax, isDiscovered, rarity
end
...
local function SI_GetQuestReward()
	...
	for index = 1, GetNumQuestLogEntries() do
		local questLogTitleText, level, questTag, suggestedGroup, isHeader, isCollapsed, isComplete, isDaily, startEvent
		questLogTitleText, level, questTag, suggestedGroup, isHeader,
		isCollapsed, isComplete, isDaily, id, startEvent = GetQuestLogTitle(index)
...
local function ShowToonTooltip(cell, arg, ...)
	...
	indicatortip:AddLine(STAT_AVERAGE_ITEM_LEVEL, ("%d "):format(t.IL or 0))
	if t.Money then
...
local function ShowHistoryTooltip(cell, arg, ...)
	...
	indicatortip:SetHeaderFont(tooltip:GetHeaderFont())
	indicatortip:SetCell(indicatortip:AddHeader(), 1, GOLDFONT .. cnt .. " " .. L["Recent Instances"] .. ": " .. FONTEND,
		"LEFT", 2)
...
local function ShowIndicatorTooltip(cell, arg, ...)
	...
	indicatortip:SetCell(nameline, 1, DifficultyString(instance, diff, toon) .. " " .. GOLDFONT .. instance .. FONTEND,
		indicatortip:GetHeaderFont(), "LEFT", 2)
	...
	if info.Extended then
		indicatortip:SetCell(indicatortip:AddLine(), 1,
			WHITEFONT .. EMPH .. L["Extended Lockout - Not yet saved"] .. EMPH .. FONTEND, "CENTER", 2)
	elseif info.Locked == false and info.ID > 0 then
		indicatortip:SetCell(indicatortip:AddLine(), 1,
			WHITEFONT .. EMPH .. L["Expired Lockout - Can be extended"] .. EMPH .. FONTEND, "CENTER", 2)
	end
	if info.Expires > 0 then
		indicatortip:AddLine(YELLOWFONT .. L["Time Left"] .. ":" .. FONTEND,
			SecondsToTime(thisinstance[toon][diff].Expires - time()))
	end
	indicatortip:SetAutoHideDelay(0.1, tooltip)
	indicatortip:SmartAnchorTo(tooltip)
	if info.Link then
...
function core:OnEnable()
	...
	self:RegisterEvent("PLAYER_LOGOUT", function()
		addon.logout = true; addon:UpdateToonData()
...
local function doExplicitReset(instancemsg, failed)
	if HasLFGRestrictions() or IsInInstance() or
		(addon:InGroup() and not IsPartyLeader() and not IsRaidLeader()) then
		return
	end
	if not failed then
...
function addon:HistoryUpdate(forcereset, forcemesg)
	...
	debug(livecnt ..
	" live instances, oldest (" ..
	(oldestkey or "none") .. ") expires in " .. oldistexp .. ". Current Zone=" .. (newzone or "nil"))
	...
	if forcemesg or (vars.db.Tooltip.LimitWarn and zoningin and livecnt >= addon.histLimit - 1) then
		...
		:format(livecnt, addon.histLimit, oldistexp))
	end
...
function core:Refresh()
	...
	if not instancesUpdated then
		addon.RefreshPending = true; return
...
function addon:ShowDetached()
	if not addon.detachframe then
		...
		f:SetScript("OnHide", function() if tooltip then
				QTip:Release(tooltip); tooltip = nil
			end end)
...
function core:ShowTooltip(anchorframe)
	...
	tooltip:SetCellScript(headLine, 1, "OnLeave",
		function()
			indicatortip:Hide(); GameTooltip:Hide()
		end)
	addon:UpdateToonData()
			...
			if inst[toon] then
				...
				for diff = 1, 4 do
					if showcol[diff] then
						...
						tooltip:SetCellScript(instancerow[instance], columns[toon .. base], "OnEnter",
							ShowIndicatorTooltip, { instance, toon, diff })
			...
			if info.Holiday then
				for toon, t in cpairs(vars.db.Toons) do
					...
					if d then
						...
						tooltip:SetCell(holidayinst[instance], columns[toon .. 1], ClassColorise(t.Class, tstr), "CENTER",
							4)
					end
				end
			end
    ...
    do
		...
		for toon, t in cpairs(vars.db.Toons) do
			if showd and columns[toon..1] and t.DailyCount > 0 then
				...
				tooltip:SetCellScript(showd, columns[toon..1], "OnLeave", function() indicatortip:Hide(); GameTooltip:Hide() end)
			end
			if showw and columns[toon..1] and weeklycnt[toon] > 0 then
				...
				tooltip:SetCellScript(showw, columns[toon..1], "OnLeave", function() indicatortip:Hide(); GameTooltip:Hide() end)
			end
		end
        end
	...
	for _, idx in ipairs(currency) do
		...
		if setting or showall then
			...
			for toon, t in cpairs(vars.db.Toons) do
				...
				local gotsome = ((ci.earnedThisWeek or 0) > 0) or ((ci.amount or 0) > 0 and showall)
				...
				for toon, t in cpairs(vars.db.Toons) do
					...
					if ci and columns[toon .. 1] then
						...
						if str then
							...
							tooltip:SetCellScript(currLine, columns[toon .. 1], "OnEnter", ShowCurrencyTooltip,
								{ toon, idx, ci })
	...
	if vars.db.Tooltip.ShowCategories then
		for category, row in pairs(categoryrow) do
			if (categories > 1 or vars.db.Tooltip.ShowSoloCategory) and categoryshown[category] then
				tooltip:SetCell(categoryrow[category], 1, YELLOWFONT .. vars.Categories[category] .. FONTEND, "LEFT",
	...
	if vars.db.Tooltip.ShowHints then
		...
		if not addon:IsDetached() then
			...
			tooltip:SetCell(hintLine, hintCol, L["|cffffff00Left-click|r to detach tooltip"], "LEFT",
				tooltip:GetColumnCount())
			hintLine, hintCol = tooltip:AddLine()
			tooltip:SetCell(hintLine, hintCol, L["|cffffff00Middle-click|r to show Blizzard's Raid Information"], "LEFT",
				tooltip:GetColumnCount())
			hintLine, hintCol = tooltip:AddLine()
			tooltip:SetCell(hintLine, hintCol, L["|cffffff00Right-click|r to configure SavedInstances"], "LEFT",
				tooltip:GetColumnCount())
		end
		hintLine, hintCol = tooltip:AddLine()
		...
		if not showall then
			hintLine, hintCol = tooltip:AddLine()
			tooltip:SetCell(hintLine, hintCol, L["Hold Alt to show all data"], "LEFT",
				math.max(1, tooltip:GetColumnCount() - 4))
