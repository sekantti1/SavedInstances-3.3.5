...
function labelPrototype:getContentHeight()
    ...
    -- Ensure both values are numbers (default to 0 if nil or invalid)
    selfWidth = type(selfWidth) == "number" and selfWidth or 0
    stringWidth = type(stringWidth) == "number" and stringWidth or 0
    
    -- Take the highest of the two
    local maxWidth = math.max(selfWidth, stringWidth)
    
    fs:SetWidth(maxWidth - (self._paddingL + self._paddingR))
    local height = fs:GetHeight()
    fs:SetWidth(0)
